...
export default function BracketPage({ id }: { id: string }) {
  ...
  // В ACTIVE режиме ВСЕГДА показываем сетку юзера
              ...
              {displayBracket[selectedRound]?.length > 0 ? (
                displayBracket[selectedRound].map((match: BracketMatch) => {
                  ...
                  if (selectedRound === 'Champion') {
                     ...
                     const state = getPlayerState(uChamp, !!myPick);
                     let bgStyle = '#1E1E1E';
                  ...
                  // --- ОБЫЧНЫЙ МАТЧ ---
                  const p1S = getPlayerState(uP1.name, clean(myPick) === clean(uP1.name));
                  const p2S = getPlayerState(uP2.name, clean(myPick) === clean(uP2.name));
                    ...
                    <div key={match.id} className={styles.matchWrapper}>
                      <div className={styles.matchContainer}>
                        ...
                        >
                          ...
                          <div className="flex gap-2 mr-2">
                            {scores.map((s: string, i: number) => (
                              <span key={i} className="text-[11px] font-mono text-[#8E8E93]">{s.split('-')[0]}</span>
                            ))}
                          </div>
                        ...
                        >
                           ...
                           <div className="flex gap-2 mr-2">
                             {scores.map((s: string, i: number) => (
                               <span key={i} className="text-[11px] font-mono text-[#8E8E93]">{s.split('-')[1]}</span>
                             ))}
                           </div>
