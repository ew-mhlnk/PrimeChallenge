...
// Вспомогательный хелпер для покрытия
const getSurfaceColor = (surface?: string) => {
    const s = surface?.toLowerCase() || '';
    if (s.includes('clay') || s.includes('грунт')) return 'bg-[#E06528]';
    if (s.includes('grass') || s.includes('трава')) return 'bg-[#3E8E41]';
    if (s.includes('hard') || s.includes('хард')) return 'bg-[#007AFF]';
    return 'bg-[#8E8E93]';
};
...
// Выносим внутренности карточки отдельно, чтобы не дублировать верстку
const CardContent = ({ tournament, statusBadge }: { tournament: Tournament; statusBadge: React.ReactNode }) => {
  return (
    <>
      {/* Левая часть: Инфо */}
    ...
    <Link href={`/tournament/${tournament.id}`} onClick={() => impact('light')} className="block w-full mb-3">
      ...
      >
        {isSlam ? (
          ...
          /* Для остальных — обычный рендеринг */
          <CardContent tournament={tournament} statusBadge={statusBadge} />
        )}
