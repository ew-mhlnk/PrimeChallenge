...
def get_match_rows(round_name: str, draw_size: int):
    ...
    if round_name not in base_map:
        return []
    start_idx, step = base_map[round_name]

    count_map = {
        "F": 1, "SF": 2, "QF": 4, "R16": 8,
        "R32": 16, "R64": 32, "R128": 64
    }
    count = count_map.get(round_name, 0)
...
async def sync_google_sheets_with_db(engine: Engine) -> None:
    print("--- STARTING SYNC (IMAGE UPDATE) ---")
    try:
    ...
    with engine.connect() as conn:
            ...
            with conn.begin_nested():
                try:
                    ...
                    if "1000" in t_type_lower:
                        draw_size = 64
                    elif "slam" in t_type_lower or ("тбш" in tag.lower() if tag else False):
                        draw_size = 128
        ...
        # Синхронизация матчей (true_draw)
                    ...
                    for round_name in rounds_order:
                        ...
                        for i, r_idx in enumerate(row_indices):
                            ...
                            if round_name == "F" and champion:
                                if is_same_player(p1, champion):
                                    winner = p1
                                elif is_same_player(p2, champion):
                                    winner = p2
