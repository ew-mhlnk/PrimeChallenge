        ...
        for tournament_id, sheet_name, starting_round in tournaments_to_sync:
            try:
                with conn.begin_nested():
                    ...
                    starting_round_order = round_order.get(starting_round, 3)
                    ...
                    while row_idx < len(data) - 1:
                        ...
                        for col_idx, round_name in round_columns.items():
                            ...
                            active_users = conn.execute(text("SELECT user_id FROM users WHERE status = 'ACTIVE'")).fetchall()  # Исправлено с id на user_id
                            if match_id:
