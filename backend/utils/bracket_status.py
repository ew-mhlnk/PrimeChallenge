...
def normalize_name(name: str) -> str:
    ...
    # 1. Убираем скобки (1), (Q) и т.д.
    n = re.sub(r'\s*\(.*?\)', '', lower_name)
    
    # 2. Оставляем буквы (включая кириллицу) и цифры
    # Было: [^a-z] - это убивало русские имена на бэке
    # Стало: [^\w] - это удаляет знаки препинания, но оставляет буквы (любого языка) и цифры
    n = re.sub(r'[^\w]', '', n)
    
    # Убираем цифры, если они не часть имени (на всякий случай, но \w оставляет цифры)
    # Если хотим только буквы:
    n = re.sub(r'\d', '', n)
...
def reconstruct_fantasy_bracket(bracket: Dict[str, List[Dict]], user_picks: List) -> Dict[str, List[Dict]]:
    ...
    for r_idx, r_name in enumerate(rounds_order):
        ...
        for match in matches:
            ...
            if not predicted:
                # Используем normalize_name, чтобы bye не зависел от регистра
                if p2 and normalize_name(p2) == "bye" and normalize_name(p1) != "tbd": predicted = p1
                elif p1 and normalize_name(p1) == "bye" and normalize_name(p2) != "tbd": predicted = p2
            
            if predicted:
                fantasy_winners[(r_name, m_num)] = predicted
                match['predicted_winner'] = predicted # <--- ВАЖНО: Добавляем поле для фронта
...
def enrich_bracket_with_status(bracket: Dict[str, List[Dict]], true_draws: List) -> Dict[str, List[Dict]]:
    ...
    for r_name in rounds_order:
        ...
        # ВАЖНО: Мы убрали блокировку по is_start_round для статусов!
        ...
        for match in bracket[r_name]:
            ...
            def get_status(u_norm, r_norm):
                ...
                # --- ИСПРАВЛЕНИЕ: Если матч уже сыгран и мы видим результат, 
                # мы ДОЛЖНЫ показать статус, даже в первом раунде.
                # Но если результат неизвестен, то в первом раунде PENDING.
