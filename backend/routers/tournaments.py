...
async def get_tournament_by_id(id: int, db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
    # ... (код получения данных без изменений) ...
    tournament = db.query(models.Tournament).filter(models.Tournament.id == id).first()
    if not tournament: raise HTTPException(status_code=404, detail="Tournament not found")
    user_id = user["id"]
    true_draws = db.query(models.TrueDraw).filter(models.TrueDraw.tournament_id == id).all()
    user_picks = db.query(models.UserPick).filter(models.UserPick.tournament_id == id, models.UserPick.user_id == user_id).all()
    ...
    try: starting_index = all_rounds.index(start_round_clean)
    except ValueError: starting_index = 2 
    rounds = all_rounds[starting_index:]
    
    # 1. Генерируем БАЗОВУЮ структуру (как в Active)
    bracket = generate_bracket(tournament, true_draws, user_picks, rounds)
    
    # 2. ЕСЛИ ТУРНИР ЗАКРЫТ:
    if tournament.status in ["CLOSED", "COMPLETED"]:
        # А. Превращаем её в Фэнтези (протаскиваем пики юзера вперед)
        bracket = reconstruct_fantasy_bracket(bracket, user_picks)
        # Б. Сравниваем с реальностью и красим
